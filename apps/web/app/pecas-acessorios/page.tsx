...
export default async function PartsPage({
  ...
  searchParams: { q?: string; categoria?: string; capacidade?: string; aplicacao?: string };
}) {
  const [categories, baseProducts] = await Promise.all([
    getCategories(ProductType.PART),
    getProducts({ type: ProductType.PART, search: searchParams.q, categorySlug: searchParams.categoria }),
  ]);
  ...
  return (
    <SiteShell>
      <section className="container-shell py-10">
        <SectionHeading title="Peças e Acessórios" subtitle="Catálogo de itens de reposição e acessórios com busca e filtros por categoria." />
        <ProductFilters
        ...
        <div className="mt-6 md:hidden">
          <div className="scroll-container -mx-4 px-4">
            {products.map((product, idx) => (
              <div key={product.id} className="scroll-item w-[280px]">
                <ProductCard product={product} hrefBase="/pecas-acessorios" index={idx} />
              </div>
            ))}
          </div>
          {products.length === 0 && <p className="text-sm text-muted">Nenhum item encontrado.</p>}
        </div>
